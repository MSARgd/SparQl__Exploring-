...
def get_country_info(country):
    query = f"""
        ...
        WHERE {{
            ...
            FILTER(REGEX(STR(?property), "http://dbpedia.org/ontology/(areaTotal|populationTotal|grossDomesticProduct)"))
        }}
    """
...
def search_country():
    ...
    if country_info:
        ...
        if "grossDomesticProduct" in country_info:
            gdp_value = float(country_info["grossDomesticProduct"])
            years = np.arange(2000, 2021)
            gdp_data = [gdp_value] * len(years)
